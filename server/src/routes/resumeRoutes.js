...
// POST /api/resumes/generate (original api)
...
// NEW Version of /generate that uses passed in data
router.post("/generate_v2", verifyJWT, extractUserId, async (req, res) => {
  try {
    const { jobId, selectedCareers, selectedEdus, selectedSkills } = req.body;
    ...
    const result = await ResumeService.generateResume(jobId, req.userId, selectedCareers, selectedEdus, selectedSkills);
