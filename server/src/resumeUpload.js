...
const resumeUpload = async (filepath) =>
{
    ...
    try {
        // Identify extension
        let resume_text = ""
        if (fileExtension === '.docx'){  // Convert DOCX file
            resume_text = await docx2Text(fileBuffer)
        }
        else if (fileExtension === '.pdf'){ // Convert PDF file
...
function docx2Text(fbuffer){
    ...
    return mammoth.extractRawText({ buffer: fbuffer })
        .then(result => {
...
async function parse(ai, resume_text) {    
    const response = await ai.models.generateContent({
        ...
        contents: "Extract the following information from this resume text and return as JSON: {" 
...
function cleanJsonResponse(responseText) {
    if (responseText.startsWith("```json") && responseText.endsWith("```")) {
      ...
      const jsonString = responseText.substring(7, responseText.length - 3).trim();
