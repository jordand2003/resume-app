        ...
        try {
            ...
            if (response.data.status === "Success") {
                ...
                if (response.data.newEntry) {
                    updatedHistory[index] = { ...formattedData, _id: response.data.data._id };
                } else {
                    updatedHistory[index] = { ...formattedData, _id: response.data.data[0]._id };
                }
            ...
            setError(
                "Failed to save career entry: " + (error.response?.data?.message || error.message)
            );
        }
    };
