...
const Dashboard = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      // Fetch Skills
      const skillResponse = await axios.get(createApiUrl("api/skills"), {
        headers: {
          Authorization: `Bearer ${token}`,
          "Cache-Control": "no-cache",
        },
  ...
  const handleDelete = async (skill) => {
    try {
      ...
      const skillResponse = await axios.delete(
        createApiUrl(`api/skills/${encodeURIComponent(skill)}`),
        {
          headers: {
            Authorization: `Bearer ${token}`,
            "Cache-Control": "no-cache",
  ...
  const addSkill_2_Db = async (newSkill) => {
    try {
      const token = await getAccessTokenSilently(); // Get token
      const skillResponse = await axios.put(
        createApiUrl(`api/skills/${newSkill}`),
        {},
        {
          headers: {
            Authorization: `Bearer ${token}`,
      ...
      setSkillError(true);
      setSkillErrorMsg(
        error.response?.data?.message || "Failed to add skill due to an error."
      );
    }
  };
  ...
  const newSkillValid = () => {
    newSkill.trim().length > 0 ||
      (() => {
        setSkillError(true);
        throw new Error("'Nothing' isn't a skill. Entry must not be empty. ");
      })();
  };
      ...
      <div>
        <a href={wd}>
          <img
            ...
            style={{ opacity: 0.005, width: "150px" }}
          />
        </a>
      </div>
    </Box>
